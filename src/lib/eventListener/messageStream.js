...
class MessageStream extends baseDataReceiver.BaseDataReceiver {
    ...
    createReceivers() {
        this._receivers = this.protocols.map((originProtocol) => {
            ...
            receiver.on('data', (data, connKey) => this.dataHandler(protocol, data, connKey));
            return receiver;
        });
        this._dataBuffers = {};
    }
     ...
     */
    dataHandler(proto, data, senderKey) {
        data = data.toString();
     ...
     * @fires MessageStream#messages
     */
    extractMessages(data, incomplete) {
