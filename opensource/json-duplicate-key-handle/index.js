 ...
 */
function validate(jsonString, valueHandler) {
  var error;
  if (typeof jsonString !== 'string') {
    ...
    try {
      ...
      _findValue(jsonString, 0, valueHandler, false);
    } catch(e) {
      error = e.message;
    }
  }
 ...
 * @returns {Object}
 */
function parse(jsonString, valueHandler) {
  if (typeof jsonString !== 'string') {
    throw new Error('Input must be a string');
  }

  // Try to find a value starting from index 0
  var value = _findValue(jsonString, 0, valueHandler, true);
  return value.value;
}
 ...
 * @param {String} str - original json string
 * @param {Number} startInd
 * @param {Boolean} valueHandler - allow duplicated keys in objects or not
 * @returns {{value: *, start: Number, end: Number}}
 * @private
 */
function _findValue(str, startInd, valueHandler, parse) {
  var len = str.length;
  var valueStartInd;
  var valueEndInd;
  var isArray = false;
  ...
  for (var i = startInd; i < len; i++) {
    ...
    if (valueStartInd === undefined) {
      ...
      if (isArray) {
        var arr = _findArray(str, i, valueHandler, parse);
        valueEndInd = arr.end;
        value = arr.value;
        break;
      } else if (isObject) {
        var obj = _findObject(str, i, valueHandler, parse);
        valueEndInd = obj.end;
        value = obj.value;
        break;
 ...
 * @param {String} str - original json string
 * @param {Number} startInd
 * @param {Boolean} valueHandler
 * @returns {{start: Number, end: Number, value: Object}}
 * @private
 */
function _findObject(str, startInd, valueHandler, parse) {
  var i = startInd;
  var sepValue = ',';
  var obj = {};
  var keys = [];
  ...
  while (sepValue === ',') {
    ...
    var value = _findValue(str, semi.end, valueHandler, parse);
    var sepIndex = _findSeparator(str, value.end);

    /**
     * TS Update - execute provided function that determines how JSON key/value pairs are stored.
     * Allows for custom logic, such as how to handle duplicate key values in a JSON string
     */
    valueHandler(keys, key, values, value);
 ...
 * @private
 */
function _findArray(str, startInd, valueHandler, parse) {
  var i = startInd;
  var sepValue = ',';
  var arr = [];
  ...
  while (sepValue === ',') {
    var value = _findValue(str, i, valueHandler, parse);
    var sepIndex = _findSeparator(str, value.end);
